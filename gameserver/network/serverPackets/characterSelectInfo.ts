...
export class CharacterSelectInfo extends BasePacket {
	constructor(characters: Player[] | undefined, player: Player) {
		...
		if(characters) {
			...
			for(let i = 0; i < characters.length; i++) {
				this.writeS(characters[i].name)
					...
					.writeD(characters[i].location.x)	// no effect ?
					.writeD(characters[i].location.y)	// no effect ?
					.writeD(characters[i].location.z)	// no effect ?
